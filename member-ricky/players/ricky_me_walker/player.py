...
def best_start_cells(start_datetime, manhattan_zones, mod, ohe, mod_me):
...
def predict_prob_trip(current_datetime, current_cell, manhattan_zones, mod, mod_me, ohe, graph):
    ...
    b = max([mod_me.predict(a), 0])
    return (odds_to_prob(max(freq_pred[0], 0)), b)
    ...
    for _ in range(k):
        ...
        for cell in list(visited.keys()):
            ...
            for neighbour in bfs.get_neighbours(graph, cell):
                ...
                (prob, earnings) = predict_func(time, neighbour)
                new_cumprob = cumprob + cumprob_bar * prob * earnings
...
def first_move(start_datetime, all_shifts, manhattan_zones, mod, ohe, mod_me):
    """
    ...
    #     start_datetime, manhattan_zones, mod, ohe, mod_me)
    # next_move = best_cell
...
def play_turn(current_datetime, current_cell, neighbours, graph,
              manhattan_zones, mod, ohe, mod_me):
    """
...
def next_shift(current_datetime, all_shifts, manhattan_zones, mod, ohe, mod_me):
    """
...
def play_game(graph, manhattan_zones, mod, ohe, mod_me):
    """
...
def main(root_path, script_path):
    ...
    with open('../model/lr_agg_me.pkl', 'rb') as handle:
        mod_me = pickle.load(handle)
    ...
    # Start playing game
    play_game(graph, manhattan_zones, mod, ohe, mod_me)
