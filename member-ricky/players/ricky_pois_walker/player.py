...
def best_start_cells(start_datetime, manhattan_zones, mod, ohe):
    ...
    poss = pd.DataFrame([(time_tuple + (zone,)) for zone in manhattan_zones.values()],
                        columns=['Pickup_wday', 'Pickup_hour', 'Pickup_minute', 'Zone'])

    freq_pred = mod.predict(poss)

    best_zone = poss.iloc[np.argmax(freq_pred), :]['Zone']
...
def predict_prob_trip(current_datetime, current_cell, manhattan_zones, mod, ohe, graph):
    ...
    poss = pd.DataFrame([key], columns=['Pickup_wday',
                                        'Pickup_hour', 'Pickup_minute', 'Zone'])

    freq_pred = mod.predict(poss).values[0]

    def odds_to_prob(freq):
        return freq / (1 + freq)

    return odds_to_prob(max(freq_pred, 0))
...
def first_move(start_datetime, all_shifts, manhattan_zones, mod, ohe):
    """
...
def next_shift(current_datetime, all_shifts, manhattan_zones, mod, ohe):
    """
...
def main(root_path, script_path):
    ...
    with open('../model/pois_agg.pkl', 'rb') as handle:
