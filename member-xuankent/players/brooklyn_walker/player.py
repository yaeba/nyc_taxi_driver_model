...
def first_move(start_datetime, brooklyn_cells):
    """
    ...
    # Start in any brooklyn cell
    next_move = random.choice(list(brooklyn_cells))
...
def play_turn(current_datetime, current_cell, neighbours, graph, brooklyn_cells):
    """
    ...
    if current_cell in brooklyn_cells:
        # Already in brooklyn, choose STAY or MOVE within brooklyn
        poss = [cell for cell in neighbours 
                if cell in brooklyn_cells] + [current_cell]
        next_move = random.choice(poss)
    else:
        # Head towards brooklyn
        costs = bfs.bfs(graph, current_cell)
        all_costs = [(costs[cell], cell) for cell in brooklyn_cells if cell in costs]

        if not all_costs:
            # brooklyn cells are not reachable
            next_move = random.choice(neighbours)
        else:
            # Move along the path to closest brooklyn cell
...
def next_shift(current_time, brooklyn_cells):
    """
...
def play_game(graph, brooklyn_cells):
    """
    ...
    while(1) :
        ...
        if(reqtype=="FIRSTMOVE"):
            action = first_move(current_datetime, brooklyn_cells)
            ...
            action=play_turn(current_datetime, current_cell, neighbours, graph, brooklyn_cells)
            print("MAST30034:" + json.dumps(action))
            sys.stdout.flush()

        elif(reqtype=="NEXTSHIFT"):
            action=next_shift(current_datetime, brooklyn_cells)
            # Create response named list that will be converted to JSON
            resp = {}
            resp["defer"]=action['defer'].strftime("%Y-%m-%dT%H:%M")
            resp["nextMove"]="REPOSITION"
...
def main(root_path):
    ...
    # Start playing game
    play_game(graph, brooklyn_cells)
