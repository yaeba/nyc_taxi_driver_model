...
def first_move(start_datetime, queens_cells):
    """
    ...
    # Start in any queens cell
    next_move = random.choice(list(queens_cells))
...
def play_turn(current_datetime, current_cell, neighbours, graph, queens_cells):
    """
    ...
    if current_cell in queens_cells:
        # Already in queens, choose STAY or MOVE within Manhattan
        poss = [cell for cell in neighbours 
                if cell in queens_cells] + [current_cell]
        next_move = random.choice(poss)
    else:
        # Head towards queens
        costs = bfs.bfs(graph, current_cell)
        all_costs = [(costs[cell], cell) for cell in queens_cells if cell in costs]

        if not all_costs:
            # queens cells are not reachable
            next_move = random.choice(neighbours)
        else:
            # Move along the path to closest queens cell
...
def next_shift(current_time, queens_cells):
    """
...
def play_game(graph, queens_cells):
    """
    ...
    while(1) :
        ...
        if(reqtype=="FIRSTMOVE"):
            action = first_move(current_datetime, queens_cells)
            ...
            action=play_turn(current_datetime, current_cell, neighbours, graph, queens_cells)
            print("MAST30034:" + json.dumps(action))
            sys.stdout.flush()

        elif(reqtype=="NEXTSHIFT"):
            action=next_shift(current_datetime, queens_cells)
            # Create response named list that will be converted to JSON
            resp = {}
            resp["defer"]=action['defer'].strftime("%Y-%m-%dT%H:%M")
            resp["nextMove"]="REPOSITION"
