...
def first_move(start_datetime, staten_island_cells):
    """
    ...
    # Start in any staten_island cell
    next_move = random.choice(list(staten_island_cells))
...
def play_turn(current_datetime, current_cell, neighbours, graph, staten_island_cells):
    """
    ...
    if current_cell in staten_island_cells:
        # Already in staten_island, choose STAY or MOVE within Manhattan
        poss = [cell for cell in neighbours 
                if cell in staten_island_cells] + [current_cell]
        next_move = random.choice(poss)
    else:
        # Head towards staten_island
        costs = bfs.bfs(graph, current_cell)
        all_costs = [(costs[cell], cell) for cell in staten_island_cells if cell in costs]

        if not all_costs:
            # staten_island cells are not reachable
            next_move = random.choice(neighbours)
        else:
            # Move along the path to closest staten_island cell
...
def next_shift(current_time, staten_island_cells):
    """
...
def play_game(graph, staten_island_cells):
    """
    ...
    while(1) :
        ...
        if(reqtype=="FIRSTMOVE"):
            action = first_move(current_datetime, staten_island_cells)
            ...
            action=play_turn(current_datetime, current_cell, neighbours, graph, staten_island_cells)
            print("MAST30034:" + json.dumps(action))
            sys.stdout.flush()

        elif(reqtype=="NEXTSHIFT"):
            action=next_shift(current_datetime, staten_island_cells)
            # Create response named list that will be converted to JSON
            resp = {}
            resp["defer"]=action['defer'].strftime("%Y-%m-%dT%H:%M")
            resp["nextMove"]="REPOSITION"
...
def main(root_path):
    ...
    # Start playing game
    play_game(graph, staten_island_cells)
